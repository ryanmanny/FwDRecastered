...
FishingBoard::FishingBoard() : dirDist(0,16) , xDist(0,BOARDWIDTH - 1) , yDist(0, BOARDHEIGHT - 1)
{
...
bool FishingBoard::spawnFish()
{
	return spawnFish(xDist(rand), yDist(rand));
}

bool FishingBoard::spawnFish(const unsigned int x, const unsigned int y)
{
	if (board[x][y].empty)
	{
...
Fish *FishingBoard::check(Pos pos)
{
	return check(pos.x, pos.y);
}

//returns the fish you shoot so you can see what's inside
Fish *FishingBoard::check(unsigned int x, unsigned int y)
{
...
bool FishingBoard::moveFish(unsigned int x, unsigned int y)
{
...
void FishingBoard::printHighlight(char highlight) const
{
...
bool FishingBoard::changeSpeed(unsigned int newSpeed) //actually changes pause length
{
